...
// Active list based on selected tab
  ...
  // ✅ Shared filter logic — applies all currently active filters to any base list.
  // Used both for the visible list (active tab) AND for computing accurate
  // tab badge counts (both tabs, regardless of which is active).
  //
  // includeTaskFilter: set false when computing counts FOR the flag chips
  // themselves (All / Direct Task / Project / Lead Task) — those need every
  // OTHER active filter (site/client/priority/status/assignedTo) applied,
  // but must not pre-filter by flag, otherwise each chip's own count would
  // only ever show against the currently selected chip.
  List<TaskListData> _applyFilters(List<TaskListData> base,
      {bool includeTaskFilter = true}) {
    var list = List<TaskListData>.from(base);

    if (includeTaskFilter && selectedTaskFilter != 'All') {
      list = list.where((t) => t.flag == selectedTaskFilter).toList();
    }
    if (selectedStatus.isNotEmpty) {
      list = list.where((t) =>
  ...
  // ✅ List actually rendered on screen — filters applied to the active tab's base list
  List<TaskListData> get filteredTaskList => _applyFilters(activeTabList);

  // ✅ Tab badge counts — filters applied to EACH tab's own base list,
  // independent of which tab is currently active. This keeps both badges
  // (and the stats row) consistent with whatever filter (site/client/
  // priority/status/etc.) is currently applied, fixing the mismatch where
  // the badge showed the unfiltered total while the list below was filtered.
  int get filteredAssignedToMeCount => _applyFilters(assignedToMeList).length;
  int get filteredAssignedByMeCount => _applyFilters(assignedByMeList).length;

  // ✅ Base list for the flag filter chips (All / Direct Task / Project /
  // Lead Task) — every active filter applied EXCEPT the flag filter itself,
  // so chip counts stay in sync with site/client/priority/status filters
  // (previously these used the unfiltered activeTabList, same class of bug
  // as the tab badges).
  List<TaskListData> get filteredTaskListForChips =>
      _applyFilters(activeTabList, includeTaskFilter: false);
